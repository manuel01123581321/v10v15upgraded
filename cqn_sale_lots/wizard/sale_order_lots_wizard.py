...
class SaleOrderLotsWizard(models.TransientModel):
    _name = 'sale.order.lots.wizard'

    product_id = fields.Many2one('product.product',"Producto")
    qty_reserved = fields.Float("Cantidad reservada", default = 0)
    qty_available = fields.Float("Cantidad en stock", default = 0)
    lots_ids = fields.One2many('sale.order.lots.line', 'wizard_id', 'Wizard')

class SaleOrderLotsLine(models.TransientModel):
    _name = 'sale.order.lots.line'

    wizard_id = fields.Many2one('sale.order.lots.wizard','Wizard')
    location_id = fields.Many2one('stock.location', 'Ubicación')
    lot_id = fields.Many2one('stock.production.lot', 'Lote')
    expiry_date = fields.Date('Fecha de caducidad')
